...
func (d *diskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		if d.readPos > 0 {
			...
			if err != nil {
				d.readFile.Close()
				d.logf(ERROR, "DISKQUEUE(%s): readOne() seek %s", d.name, curFileName)
				d.readFile = nil
				return nil, err
			}
		}
	...
	if err != nil {
		d.readFile.Close()
		d.logf(ERROR, "DISKQUEUE(%s): readOne() read msg size %s", d.name, d.readFile.Name())
		d.readFile = nil
		return nil, err
	}
	...
	if err != nil {
		d.readFile.Close()
		d.logf(ERROR, "DISKQUEUE(%s): readOne() read data %s", d.name, d.readFile.Name())
		d.readFile = nil
		return nil, err
	}
	...
	d.logf(INFO, "DISKQUEUE(%s): readOne() nextReadFileNum:%d nextReadPos:%d", d.name, d.nextReadFileNum, d.nextReadPos)
...
func (d *diskQueue) moveForward() {
	...
	d.logf(INFO,
		"DISKQUEUE(%s) moveForward() readFileNum:%d readPos:%d",
		d.name, d.readFileNum, d.readPos)
...
// 处理的消息每条消息依次移动
func (d *diskQueue) HandForward(dataList [][]byte) {
	for _, data := range dataList {
		dataTotalLen := int64(4 + len(data))
		nextPos := d.handPos + dataTotalLen
		oldHandFileNum := d.handFileNum
		nextFileNum := d.handFileNum
		if nextPos >= d.maxBytesPerFile {
			nextFileNum++
			nextPos = 0
			err := os.Remove(d.fileName(oldHandFileNum))
			if err != nil {
				// TODO handle err
				fmt.Println("remove file error:", err)
			}
		}
		d.handFileNum = nextFileNum
		d.handPos = nextPos
		err := d.PersistMetaReadData()
		if err != nil {
			// TODO handle err
			fmt.Println("persist meta data error:", err)
		}
	}
}
